...
void read_bin(std::string bin_path, Eigen::MatrixXf& cloud) {
    ...
    for (int i = 0; i < num_points; i++) {
        double x = buffer[i * 4];
        double y = buffer[i * 4 + 1];
        double z = buffer[i * 4 + 2];
        if (x > 150 || fabs(y) > 10 || z > 6) continue;
...
void addNormals(Eigen::MatrixXf normals, std::shared_ptr<geometry::PointCloud> geo) {
    for (int i = 0; i < normals.rows(); i++) {
        geo->normals_.push_back(
                Eigen::Vector3d(normals.row(i)(0), normals.row(i)(1), normals.row(i)(2)));
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        ...
        std::cout << "\033[1;33mNo point cloud file path specified; defaulting to the test "
                     "directory. \033[0m"
                  << std::endl;
    } else {
        input_cloud_filepath = argv[1];
        std::cout << "\033[1;32mLoading point cloud files from " << input_cloud_filepath
                  << "\033[0m" << std::endl;
    }
    if (!fs::exists(input_cloud_filepath)) {
        std::cout << "\033[1;31mERROR HERE: maybe wrong data file path, please check the path or "
                     "remove argv to run default one. \033[0m"
    ...
    const int points_num = geo_ground->points_.size();
    const int points_color_num = geo_ground->.size();
    printf();
