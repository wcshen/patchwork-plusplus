...
void PatchWorkpp::estimateGround(Eigen::MatrixXf cloud_in) {
    ...
    for (int zone_idx = 0; zone_idx < params_.num_zones; ++zone_idx) {
        ...
        for (int ring_idx = 0; ring_idx < params_.num_rings_each_zone[zone_idx]; ++ring_idx) {
            for (int sector_idx = 0; sector_idx < params_.num_sectors_each_zone[zone_idx];
                ...
                sort(zone[ring_idx][sector_idx].begin(),
                ...
                /*
                    About 'is_heading_outside' condition, heading should be smaller than 0
    ...
    if (params_.verbose) {
        cout << "Time taken : " << time_taken_ / double(1000000)
             ...
             << t_czm / double(1000) << "(czm) + " << t_sort / double(1000) << "(sort) + "
             << t_pca / double(1000) << "(pca) + " << t_gle / double(1000) << "(estimate)" << endl;
...
void PatchWorkpp::reflected_noise_removal(Eigen::MatrixXf& cloud_in) {
    ...
    for (int i = 0; i < cloud_in.rows(); i++) {
        double r = sqrt(cloud_in.row(i)(0) * cloud_in.row(i)(0) +
                        cloud_in.row(i)(1) * cloud_in.row(i)(1));
        ...
        if (ver_angle_in_deg < params_.RNR_ver_angle_thr && z < -params_.sensor_height - 0.8 &&
            ...
            cloud_nonground_.push_back(
                    PointXYZ(cloud_in.row(i)(0), cloud_in.row(i)(1), cloud_in.row(i)(2)));
...
void PatchWorkpp::pc2czm(const Eigen::MatrixXf& src, std::vector<Zone>& czm) {
    ...
    int num_ring_0 = params_.num_rings_each_zone[0],
        num_sector_0 = params_.num_sectors_each_zone[0];
    int num_ring_1 = params_.num_rings_each_zone[1],
        num_sector_1 = params_.num_sectors_each_zone[1];
    int num_ring_2 = params_.num_rings_each_zone[2],
        num_sector_2 = params_.num_sectors_each_zone[2];
    int num_ring_3 = params_.num_rings_each_zone[3],
        num_sector_3 = params_.num_sectors_each_zone[3];
    ...
    for (int i = 0; i < src.rows(); i++) {
        ...
        if ((r <= max_range) && (r > min_range)) {
            ...
            if (r < min_range_1) {  // In First rings
                ring_idx =
                        min(static_cast<int>(((r - min_range_0) / ring_sizes_[0])), num_ring_0 - 1);
                ...
                ring_idx =
                        min(static_cast<int>(((r - min_range_1) / ring_sizes_[1])), num_ring_1 - 1);
                ...
                ring_idx =
                        min(static_cast<int>(((r - min_range_2) / ring_sizes_[2])), num_ring_2 - 1);
                sector_idx = min(static_cast<int>((theta / sector_sizes_[2])), num_sector_2 - 1);
                czm[2][ring_idx][sector_idx].emplace_back(PointXYZ(x, y, z));
            } else {  // Far!
                ring_idx =
                        min(static_cast<int>(((r - min_range_3) / ring_sizes_[3])), num_ring_3 - 1);
    ...
    if (params_.verbose)
        cout << "\033[1;33m"
             << "PatchWorkpp::pc2czm() - Divides pointcloud into the concentric zone model "
                "successfully"
             << "\033[0m" << endl;
}
