...
struct Params {
    ...
    Params() {
        ...
        num_zones = 10;   // Setting of Concentric Zone Model(CZM)
        num_rings_of_interest =
                4;        // Number of rings to be checked with elevation and flatness values.

        RNR_ver_angle_thr = -15.0;  // Noise points vertical angle threshold. Downward rays of LiDAR
                                    // are more likely to generate severe noise points.
        ...
        max_range = 150.0;  // max_range of ground estimation area
        min_range = 2.7;    // min_range of ground estimation area
        uprightness_thr =
                0.707;      // threshold of uprightness using in Ground Likelihood Estimation(GLE).
